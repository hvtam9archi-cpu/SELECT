...
namespace UnifiedAutoCADTools.UI
{
	public class FilterDialog : Form
	{
		...
		public FilterDialog(bool isBlock)
		{
			...
			cbLayer = CreateCheck("Layer", x, y, _lastLayer); y += 30;
			cbColor = CreateCheck("Color", x, y, _lastColor); y += 30;
			cbLinetype = CreateCheck("Linetype", x, y, _lastLinetype); y += 30;
			cbBlock = CreateCheck("Block Name", x, y, _lastBlock);
			...
			btnOK = new Button { Text = "OK", Location = new Point(30, y), DialogResult = DialogResult.OK };
			btnCancel = new Button { Text = "Cancel", Location = new Point(120, y), DialogResult = DialogResult.Cancel };

			this.FormClosing += (s, e) =>
			{
				if (this.DialogResult == DialogResult.OK)
				{
					_lastLayer = cbLayer.Checked;
					_lastColor = cbColor.Checked;
					_lastLinetype = cbLinetype.Checked;
					_lastBlock = cbBlock.Checked;
				}
			};
		...
		{
			return new CheckBox { Text = text, Location = new Point(x, y), Checked = isChecked, AutoSize = true };
		}
	}
}